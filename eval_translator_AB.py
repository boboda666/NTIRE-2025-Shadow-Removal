...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_dir", default="/root/autodl-tmp/results/DNSR-wsrd2-r4",
    ...
    test_il = PairedImageSet('/root/autodl-tmp/ntire25_sh_rem_test_inp/', 'test', size=None, use_mask=False, aug=False)
    ...
    with torch.no_grad():
        ...
        for idx, (B_img, AB_mask, A_img) in enumerate(val_dataloader):
            ...
            # 输出单张图像耗时
            print(f"Image {idx + 1} processed in: {elapsed:.4f} seconds")
        ...
        # 计算平均耗时
        avg_time = total_time / len(test_il)
        print(f"\nAverage time per image: {avg_time:.4f} seconds")
