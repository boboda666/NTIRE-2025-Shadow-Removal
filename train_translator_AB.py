...
os.environ['TORCH_HOME'] = "/root/autodl-tmp/loaded_models/"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_checkpoint", type=int, default=10, help="checkpoint for visual inspection")
    parser.add_argument("--model_dir", default="/root/autodl-tmp/checkpoints/DNSR-wsrd2",
                        help="Path of destiantion directory for the trained models")
    parser.add_argument("--image_dir", default="/root/autodl-tmp/results/DNSR-wsrd2",
                        help="Path for the directory used to save the output test images")
    opt = parser.parse_args()
    opt.mask_weight = 1.0  # 或者你需要的值
    ...
    train_il = PairedImageSet('/root/autodl-tmp/ntire2025_sh_rem_train/', 'train', size=(opt.img_height, opt.img_width), use_mask=False, aug=True)
    test_il = PairedImageSet('/root/autodl-tmp/ntire2025_sh_rem_train/', 'test', size=None, use_mask=False, aug=False)
        ...
        if (epoch + 1) % opt.valid_checkpoint == 0 or epoch in [0, 1]:
            with torch.no_grad():
                ...
                for idx, (B_img, AB_mask, A_img) in enumerate(val_dataloader):
                    ...
                    if torch.allclose(out, gt, atol=1e-6):
                        rmse, psnr = float('inf'), float('inf')  # 或者其他处理方式
                    else:
